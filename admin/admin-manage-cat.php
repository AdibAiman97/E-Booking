<?php
  ...
  $aid=$_SESSION['a_id'];
   ...
   // Delete Logic
   if (isset($_GET['del'])) {
    $user_id = intval($_GET['del']); // Sanitize input to avoid SQL injection
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>window.location.href='admin-manage-user.php';</script>";
    } else {
        echo "<script>alert('Failed to Delete User. Try Again');</script>";
    }
}
?>
<!DOCTYPE html>
        ...
        <tbody>
          ...
          ?>
            <tr>
              ...
              <td><?php echo $row->u_cat_name; ?></td>
              <td><?php echo $row->u_cat_gender; ?></td>
              <td><?php echo $row->u_cat_weight . ' kg'; ?></td>
              <td>
                <a href="admin-manage-single-cat.php?u_id=<?php echo $row->u_id; ?>" class="btn btn-success btn-sm" style="background-color: green; color: white; border: none;">
                  <i class="fa fa-user-edit"></i> Update
                </a>
                <a href="admin-manage-user.php?del=<?php echo $row->u_id; ?>" class="btn btn-danger btn-sm">
